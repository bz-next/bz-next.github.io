...
# Cross-compiling with Emscripten from Linux

Note: This is not yet fully supported.
...
Install `c-ares`, `zlib`, and `libpng` in your emscripten distribution. The process is analogous to installing these libraries for MinGW64, except that instead of using
`mingw64-cmake`, you use `emcmake cmake`, and instead of `mingw64-configure`, you use `emconfigure ./configure`. `emcmake` will correctly set your install prefix to the
emscripten distribution's sysroot. For configure scripts, specify `--prefix=path_to_emsdk/upstream/emscripten/cache/sysroot`.
